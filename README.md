...
Кнопка входа | | Текст кнопки "Войти". Первая буква заглавная, останые строчные. По нажатию на кнопку происходит первичная валидация введнных данных. Если валидация прошла успешно данные отправляются на сервер, если валидация не проходит выводятся сообщения об ошибках под каждым полем ввода во всплывающих tooltips соответсвенно. Сервер получая данные либо авторизует пользователя на сайте при нахождении в базе пары логин/пароль и обновляет страницу, либо возвращает соответвующую ошибку. Текст ошибки выводится под формой. Страница при этом не перезагружается, данные приходят в фоном режиме.
